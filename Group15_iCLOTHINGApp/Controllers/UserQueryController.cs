...
namespace Group15_iCLOTHINGApp.Controllers
{
    public class UserQueryController : Controller
    {
        ...
        public ActionResult Index(bool add_removeFlag=false, string productPrice="", string productID="", string productName="", int quantity = 1)
        {
            bool success = false;
            ...
            else if(!productPrice.Equals("") && !productID.Equals("") && !productName.Equals(""))
            {
                if (Session["UserID"] == null)
                    return RedirectToAction("UserLogin", "UserLogin");
                Random rnd = new Random();  
                OrderStatus order = new OrderStatus();
                order.statusID = rnd.Next(1000, 9999);
